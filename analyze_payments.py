...
Return your analysis as a structured JSON with the following fields:
{
  "summary": "Brief description of what the payment was for, including the price and currency",
  "location": "Likely location of the purchase",
  "time_context": "Temporal description",
  "habitual": true/false,
  "tags": ["tag1", "tag2", ...],
...
def analyze_payments(payments, prompt, verbose=False):
    """
    Analyze one or multiple payment JSONs and return their summaries.
    
    Args:
        payments: Single payment JSON or string or list of payment JSONs or strings
    ...
    """
    if verbose:
        print(f"Analyzing {len(payments)} payments...")
    global CLIENT
    if CLIENT is None:
        CLIENT = create_client()

    if isinstance(payments, str) or isinstance(payments, dict):
        payments = [payments]
    
    # Convert any dict payments to JSON strings
    payments = [json.dumps(payment) if isinstance(payment, dict) else payment for payment in payments]
